...
make_model_matrix <- function(x, label = NA) {
  x <- data.matrix(x)
  if (!is.na(label)) {
    y <- as.numeric(as.character(label))
...
train <- model_df %>% dplyr::filter(set == 'train') %>% dplyr::filter(inds)
test <- model_df %>% dplyr::filter(set == 'train') %>% dplyr::filter(!inds)

trainx <- make_model_matrix(x = train %>% remove_cols(), label = train$reordered)

weight <- sum(train$reordered == 0) / sum(train$reordered == 1)
...
bst <- xgboost(data = trainx, params = params, 
               feval = evalerror, nrounds = 50)

testx <- make_model_matrix(x = test %>% remove_cols(), label = test$reordered)
