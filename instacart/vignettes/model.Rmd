...
model_df <- dataset %>%
  ...
  dplyr::left_join(y = cust_profile %>%
                     dplyr::select(user_id, user_order_freq, cart_size, 
                                   user_perc_reorder, customer_group),
                   by = 'user_id') %>%
  ...
  dplyr::mutate(reordered = as.factor(reordered),
                ...
                customer_group = as.factor(customer_group),
...
bst <- xgboost(data = x, label = y, 
               objective = 'binary:logistic', nround = 10)
