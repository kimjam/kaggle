...
cust_profile <- cust_df %>% 
  dplyr::select(user_id, days, cart_size, perc_reordered, perc_department) %>% 
...
user_prod <- tmp %>%
  dplyr::group_by(user_id, product_id) %>%
  dplyr::summarise(user_prod_avg_days_reordered = mean(days_since_prior_order[reorder == 1]),
...
prod_df <- tmp %>%
  ...
  dplyr::summarise(prod_avg_days_reordered = mean(days_since_prior_order[reorder == 1]),
...
department_df <- tmp %>%
  ...
  dplyr::summarise(department_avg_days_reordered = mean(days_since_prior_order[reorder == 1]),
...
aisle_df <- tmp %>%
  ...
  dplyr::summarise(aisle_avg_days_reordered = mean(days_since_prior_order[reorder == 1]),
